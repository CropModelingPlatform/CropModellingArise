...
def to_csv_xr(ds, bbox, f_out):
    print('to_csv_xr')
    #dst = ds.sel(lon=slice(bbox[0], bbox[1]), lat=slice(bbox[1], bbox[2]))
    dst = ds.isel[slice(exp, exp+sp_size)]
...
for i in range(0,1,1):
    ...
    with xr.open_dataset(f_in) as ds:
