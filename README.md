...
### Process to launch the workflow with regard to the allocated resources to ARISE
...
```
  ...
  **Launch merge.pbs job**
  ...
  After launching the job for all models and management practices, it requires to lauch `merged.pbs` that allows to merge all the results from each model with different simulation options and management practices.
